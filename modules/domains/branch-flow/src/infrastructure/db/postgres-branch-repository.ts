...
export class PostgresBranchRepository implements BranchRepository {
  ...
  async getBranchCapacityAggregates(tenantId: string, serviceId?: string): Promise<BranchCapacityAggregate[]> {
    ...
    const activeBookingsByBranch = new Map<string, number>();
    for (const appointment of apptRows) {
      if (!appointment.branchId) continue;
      activeBookingsByBranch.set(
        appointment.branchId,
        (activeBookingsByBranch.get(appointment.branchId) ?? 0) + 1,
      );
    }
    ...
    for (const row of branchRows) {
      ...
      aggregates.push({
        ...
        activeBookingsCount: activeBookingsByBranch.get(row.id) ?? 0,
        offeredServiceIds,
      });
    }
