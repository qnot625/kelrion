...
export function registerWorkforceLifecycleRoutes(
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (
        ...
        !LEAVE_TYPES.has(body.type as LeaveType) ||
        ...
        return reply
          .code(400)
          .send({ error: "type, startDate, endDate and reason are required" });
      }
      try {
        ...
        if (handled) return reply.code(handled.status).send({ error: handled.message });
        throw error;
      }
    },
  );
  ...
  app.get(
    "/lifecycle-plans",
    { preHandler: requirePermission("lifecycle:view") },
    async (request, reply) => {
      const canManage = hasPermission(request.auth!.roles, "lifecycle:manage");
      return reply.send(
        await service.listLifecyclePlans(
          request.tenant!.tenantId,
