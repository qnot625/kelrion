...
test("branch API: owner updates status, hours, closures and department capacity", async () => {
  const app = buildServer(createAppContext());
  ...
  const created = await app.inject({
    method: "POST", url: "/branches", headers,
    payload: { name: "Central", slug: "central", address: "1 Main Street", latitude: 6.45, longitude: 3.4 },
  });
  const branch = created.json() as { id: string };

  const updated = await app.inject({ method: "PATCH", url: `/branches/${branch.id}`, headers, payload: { status: "inactive", name: "Central Office" } });
  assert.equal(updated.statusCode, 200, updated.body);
  assert.equal((updated.json() as { status: string }).status, "inactive");

  const hours = await app.inject({
    method: "PUT", url: `/branches/${branch.id}/operating-windows`, headers,
    payload: { windows: [{ dayOfWeek: 1, openMinutes: 480, closeMinutes: 1020 }] },
  });
  assert.equal(hours.statusCode, 200, hours.body);

  const overlapping = await app.inject({
    method: "PUT", url: `/branches/${branch.id}/operating-windows`, headers,
    payload: { windows: [{ dayOfWeek: 1, openMinutes: 480, closeMinutes: 720 }, { dayOfWeek: 1, openMinutes: 600, closeMinutes: 900 }] },
  });
  assert.equal(overlapping.statusCode, 400, overlapping.body);

  const closure = await app.inject({
    method: "POST", url: `/branches/${branch.id}/holidays`, headers,
    payload: { name: "Maintenance", startAt: "2026-08-10T00:00:00Z", endAt: "2026-08-11T00:00:00Z" },
  });
  assert.equal(closure.statusCode, 201, closure.body);
  const holiday = closure.json() as { id: string };
  assert.equal((await app.inject({ method: "GET", url: `/branches/${branch.id}/holidays`, headers })).json().length, 1);
  assert.equal((await app.inject({ method: "DELETE", url: `/holidays/${holiday.id}`, headers })).statusCode, 204);

  const departmentResponse = await app.inject({
    method: "POST", url: `/branches/${branch.id}/departments`, headers,
    payload: { name: "Reception", slug: "reception", capacity: 3 },
  });
  const department = departmentResponse.json() as { id: string };
  const capacityUpdate = await app.inject({ method: "PATCH", url: `/departments/${department.id}`, headers, payload: { capacity: 7 } });
  assert.equal(capacityUpdate.statusCode, 200, capacityUpdate.body);
  assert.equal((capacityUpdate.json() as { capacity: number }).capacity, 7);
  assert.equal((await app.inject({ method: "DELETE", url: `/departments/${department.id}`, headers })).statusCode, 204);
});
