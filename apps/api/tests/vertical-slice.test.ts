...
test("customer visit journey: book, check in and complete an appointment for one tenant", async () => {
  ...
  const b1 = await app.inject({ method: "POST", url: "/branches", headers: { "x-tenant-slug": "acme-clinics", authorization: `Bearer ${token}` }, payload: { name: "Downtown", slug: "downtown", address: "123 Main St", latitude: 51, longitude: -0.1, status: "active" } }).then(r => r.json() as {id: string});
  await app.inject({ method: "POST", url: `/branches/${b1.id}/operating-windows`, headers: { "x-tenant-slug": "acme-clinics", authorization: `Bearer ${token}` }, payload: [{ dayOfWeek: 6, openMinutes: 480, closeMinutes: 1020 }] }); // Saturday
  await app.inject({ method: "POST", url: `/branches/${b1.id}/departments`, headers: { "x-tenant-slug": "acme-clinics", authorization: `Bearer ${token}` }, payload: { name: "D1", slug: "d1", capacity: 1 } });
  const s1 = await app.inject({ method: "POST", url: "/services", headers: { "x-tenant-slug": "acme-clinics", authorization: `Bearer ${token}` }, payload: { code: "S1", name: "S1", durationMinutes: 30 } }).then(r => r.json() as {id: string});
  await app.inject({ method: "POST", url: `/branches/${b1.id}/services`, headers: { "x-tenant-slug": "acme-clinics", authorization: `Bearer ${token}` }, payload: { serviceId: s1.id } });
...
test("tenant isolation: another tenant cannot see or act on the first tenant's appointment", async () => {
  ...
  const b1 = await app.inject({ method: "POST", url: "/branches", headers: { "x-tenant-slug": "acme-clinics", authorization: `Bearer ${acmeToken}` }, payload: { name: "Downtown", slug: "downtown", address: "123 Main St", latitude: 51, longitude: -0.1, status: "active" } }).then(r => r.json() as {id: string});
  await app.inject({ method: "POST", url: `/branches/${b1.id}/operating-windows`, headers: { "x-tenant-slug": "acme-clinics", authorization: `Bearer ${acmeToken}` }, payload: [{ dayOfWeek: 6, openMinutes: 480, closeMinutes: 1020 }] }); // Saturday
  await app.inject({ method: "POST", url: `/branches/${b1.id}/departments`, headers: { "x-tenant-slug": "acme-clinics", authorization: `Bearer ${acmeToken}` }, payload: { name: "D1", slug: "d1", capacity: 1 } });
  const s1 = await app.inject({ method: "POST", url: "/services", headers: { "x-tenant-slug": "acme-clinics", authorization: `Bearer ${acmeToken}` }, payload: { code: "S1", name: "S1", durationMinutes: 30 } }).then(r => r.json() as {id: string});
  await app.inject({ method: "POST", url: `/branches/${b1.id}/services`, headers: { "x-tenant-slug": "acme-clinics", authorization: `Bearer ${acmeToken}` }, payload: { serviceId: s1.id } });
