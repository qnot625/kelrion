import {
  BarChart3, Bell, BriefcaseBusiness, CalendarDays, ChevronDown, Command, Gauge,
  LayoutDashboard, Menu, MessageSquareWarning, Plus, Search, ShieldCheck, TicketCheck, UserCog, X,
  type LucideIcon,
} from "lucide-react";
...
export type RouteKey = "dashboard" | "appointments" | "queue" | "users" | "cases" | "executive" | "recruitment" | "audit" | "reports";
...
export function Shell({ session, route, onRoute, onSignOut, children, apiReachable }: {
  ...
  const navigate = (next: RouteKey) => { onRoute(next); setMobileOpen(false); setCommandOpen(false); setQuery(""); };
  ...
  return <div className="app-shell">
    ...
    <aside className={`sidebar ${mobileOpen ? "open" : ""}`}>
      ...
      <nav aria-label="Primary navigation"><small className="nav-label">Workspace</small>{nav.map(({ key, label, icon: Icon, badge }) => <button key={key} className={route === key ? "active" : ""} onClick={() => navigate(key)}><Icon size={18}/><span>{label}</span>{badge && <em>{badge}</em>}</button>)}</nav>
    ...
    <section className="workspace">
      <header className="topbar"><button className="menu-button" onClick={() => setMobileOpen(true)} aria-label="Open navigation"><Menu size={20}/></button><button className="global-search" onClick={() => setCommandOpen(true)}><Search size={16}/><span>Search Klerion or run a command</span><kbd>⌘ K</kbd></button><div className="topbar-actions"><span className={`connection ${session.mode === "demo" ? "demo" : apiReachable ? "online" : "offline"}`}><i/>{session.mode === "demo" ? "Preview" : apiReachable === null ? "Checking API" : apiReachable ? "Connected" : "Offline"}</span><button className="primary compact"><Plus size={15}/>Create</button><button className="icon-button" aria-label="Notifications"><Bell size={18}/><i/></button><div className="profile-anchor"><button className="profile-button" onClick={() => setProfileOpen(!profileOpen)}>{initials}</button>{profileOpen && <div className="profile-menu"><strong>{name}</strong><small>{session.email}</small><button onClick={onSignOut}>Sign out</button></div>}</div></div></header>
      <main id="main-content" className="content"><div className={`data-banner ${session.mode}`}><ShieldCheck size={16}/>{session.mode === "demo" ? "Interactive preview: representative records are shown." : "Live authentication is connected. Unfinished module records are clearly marked as preview data."}</div>{children}</main>
    </section>
